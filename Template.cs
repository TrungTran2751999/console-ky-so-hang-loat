...
namespace ConsoleKySo
{
    ...
    {
        private static string domain_api = "https://gwsca.vnpt.vn";//"https://rmgateway.vnptit.vn";
        ...
        private static string uid = "THAY DOI";
        // private static string password = "";


        private static string _pdfInput = @"C:\Users\Administrator\Desktop\testCA\sample.pdf";
        private static string _pdfSignedPath = @"C:\Users\Administrator\Desktop\testCA\sample_signed.pdf";

        private static string _pdfInput1 = @"C:\Users\Administrator\Desktop\testCA\test.pdf";
        private static string _pdfSignedPath1 = @"C:\Users\Administrator\Desktop\testCA\test_signed.pdf";

        private static string _xmlInput = @"C:\Users\nguye\OneDrive\Desktop\testCA\DNPA.xml";
        private static string _xmlSignedPath = @"C:\Users\nguye\OneDrive\Desktop\testCA\DNPA_signed.xml";
        ...
        {
            var userCert = _getAccountCert(domain_api + "/sca/sp769/v1/credentials/get_certificate");
            ...
            byte[] unsignData1 = null;
            try
            {
                unsignData = File.ReadAllBytes(_pdfInput);
                unsignData1 = File.ReadAllBytes(_pdfInput1);
            }
            catch (Exception ex)
            {
                //_log.Error(ex);
            ...
            List<string> data_to_be_signs = new List<string>();
            for (int i = 0; i < 2; i++)
            {
                byte[] data;
                ...
                if (i == 0)
                    ...
                    data = unsignData1;
                signer = HashSignerFactory.GenerateSigner(data, certBase64, null, HashSignerFactory.PDF);
                ...
                data_to_be_signs.Add(data_to_be_sign);
                signers.Add(signer);
            }
            DataSign dataSign = _sign(domain_api + "/sca/sp769/v1/signatures/sign", data_to_be_signs, userCert.user_certificates[0].serial_number);
            //   DataSign dataSign = _sign(domain_api + "/sca/sp769/v1/signatures/sign", Convert.ToBase64String(unsignData), userCert.user_certificates[0].serial_number);
            ...
            List<string> datasigneds = new List<string>();
            while (count < 30 && !isConfirm)
            {
                transactionStatus = _getStatus(string.Format(domain_api + "/sca/sp769/v1/signatures/sign/{0}/status", dataSign.transaction_id));
                if (transactionStatus.signatures != null)
                {
                    for (int i = 0; i < transactionStatus.signatures.Count; i++)
                    {
                        datasigneds.Add(transactionStatus.signatures[i].signature_value);
                    }
            ...
            {
                if (!signers[j].CheckHashSignature(datasigneds[j]))
                {
                ...
                byte[] signed = signers[j].Sign(datasigneds[j]);
                string _pdfSignedPathTem = "";
                if (j == 0)
                    _pdfSignedPathTem = _pdfSignedPath;
                else
                    _pdfSignedPathTem = _pdfSignedPath1;
                File.WriteAllBytes(_pdfSignedPathTem, signed);
            }
        ...
        private static void _signSmartCAXML()
        {
            ...
            ((XmlHashSigner)signer).SetParentNodePath("/HOC_BA");


            var hashValue = signer.GetSecondHashAsBase64();

            List<string> data_to_be_sign = null;// BitConverter.ToString(Convert.FromBase64String(hashValue)).Replace("-", "").ToLower();

            DataSign dataSign = _sign(domain_api + "/sca/sp769/v1/signatures/sign", data_to_be_sign, userCert.user_certificates[0].serial_number);
        ...
        private static DataSign _sign(String uri, List<string> data_to_be_signed, String serialNumber)
        {


            var sign_files = new List<SignFile>();
            for (int i = 0; i < data_to_be_signed.Count; i++)
            {
                var sign_file = new SignFile();
                sign_file.data_to_be_signed = data_to_be_signed[i];
                sign_file.doc_id = "test " + i;
            ...
            var response = Query(new ReqSign
            {
                ...
                transaction_desc = "Ký Test từ Nguyễn Hữu Minh",
